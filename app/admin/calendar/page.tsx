...
import { ChevronLeft, ChevronRight, Loader2, Save, X, BedDouble, Settings, IndianRupee, Utensils } from "lucide-react";
...
export default function AdminCalendar() {
  ...
  const [pricingForm, setPricingForm] = useState<any>(DEFAULT_PRICING);
  const [isSaving, setIsSaving] = useState(false);

  const handlePrevMonth = () => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1));
  const handleNextMonth = () => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1));

  // Live Listener for Global Pricing
  useEffect(() => {
    const unsubPricing = onSnapshot(doc(db, "settings", "global_pricing"), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setGlobalPricing({
          ...
          mealPlans: { ...DEFAULT_PRICING.mealPlans, ...(data.mealPlans || {}) }
        });
      }
    });
    return () => unsubPricing();
  ...
  // Live Listener for Monthly Inventory
  useEffect(() => {
    setIsLoading(true);
  ...
  const openPricingModal = () => {
    setPricingForm(JSON.parse(JSON.stringify(globalPricing)));
  ...
  const handleSaveDay = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await setDoc(doc(db, "room_inventory", `${roomType}_${editingDate}`), {
        date: editingDate,
        roomType: roomType,
  ...
  const handleSavePricing = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        kingSize: {
          onePerson: Number(pricingForm.kingSize.onePerson) || 0,
          twoPerson: Number(pricingForm.kingSize.twoPerson) || 0,
          extraChild: Number(pricingForm.kingSize.extraChild) || 0,
        },
        doubleBed: {
          onePerson: Number(pricingForm.doubleBed.onePerson) || 0,
          twoPerson: Number(pricingForm.doubleBed.twoPerson) || 0,
          threePerson: Number(pricingForm.doubleBed.threePerson) || 0,
          fourPerson: Number(pricingForm.doubleBed.fourPerson) || 0,
          extraChild: Number(pricingForm.doubleBed.extraChild) || 0,
        },
        mealPlans: {
          ep: Number(pricingForm.mealPlans.ep) || 0,
          cp: Number(pricingForm.mealPlans.cp) || 0,
          map: Number(pricingForm.mealPlans.map) || 0,
          ap: Number(pricingForm.mealPlans.ap) || 0,
        },
        updatedAt: new Date().toISOString()
      };

      await setDoc(doc(db, "settings", "global_pricing"), payload, { merge: true });
      await purgeWebsiteCache();
    ...
    <div className="space-y-6 pb-20 p-2 md:p-6">
      <div className="bg-white p-4 md:p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          <button onClick={openPricingModal} className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-3 bg-slate-900 text-white rounded-lg font-bold hover:bg-slate-800 transition-colors">
          ...
          <select value={roomType} onChange={(e) => setRoomType(e.target.value)} className="w-full sm:w-48 px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg font-bold text-gray-700 focus:outline-none focus:border-brand-primary">
            {ROOM_CATEGORIES.map(room => <option key={room} value={room}>{room}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="relative min-h-[400px]">
          ...
          <div className="grid grid-cols-7 auto-rows-fr">
            {Array.from({ length: firstDayOfMonth }).map((_, i) => <div key={`empty-${i}`} className="min-h-[80px] md:min-h-[120px] border-b border-r border-gray-100 bg-gray-50/30" />)}
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div key={dateStr} onClick={() => openEditor(dateStr, isPast)} className={`min-h-[80px] md:min-h-[120px] border-b border-r border-gray-100 p-1 md:p-2 transition-colors flex flex-col group ${isPast ? "bg-gray-100/50 opacity-60 cursor-not-allowed" : "cursor-pointer hover:bg-gray-50"} ${isToday ? "bg-orange-50/30" : ""}`}>
                  ...
                  <div className="mt-auto flex flex-col gap-1">
                    {data ? <div className={`text-[9px] md:text-xs font-bold px-1 py-1 rounded text-center ${isAvailable ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"} ${isPast ? "grayscale" : ""}`}>{isAvailable ? `${data.available} Left` : "Sold Out"}</div> : <div className="text-[9px] md:text-[10px] text-gray-400 font-bold px-1 py-1 text-center bg-gray-200/50 rounded">0 Left</div>}
                  </div>
                </div>
              );
            })}
              ...
              <form onSubmit={handleSavePricing} className="p-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="number" min="0" value={pricingForm.kingSize?.onePerson ?? ""} onChange={e => setPricingForm((p: any) => ({...p, kingSize: {...p.kingSize, onePerson: e.target.value}}))} className="w-full p-2 border rounded mt-1 font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-gray-500 uppercase">2 Persons (₹)</label>
                        <input type="number" min="0" value={pricingForm.kingSize?.twoPerson ?? ""} onChange={e => setPricingForm((p: any) => ({...p, kingSize: {...p.kingSize, twoPerson: e.target.value}}))} className="w-full p-2 border rounded mt-1 font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                      </div>
                    </div>
                    <div className="bg-orange-50 p-3 rounded-lg border border-orange-100">
                      <label className="text-xs font-bold text-orange-800 uppercase">Extra Child Rate (₹)</label>
                      <input type="number" min="0" value={pricingForm.kingSize?.extraChild ?? ""} onChange={e => setPricingForm((p: any) => ({...p, kingSize: {...p.kingSize, extraChild: e.target.value}}))} className="w-full p-2 border border-orange-200 rounded mt-1 font-bold text-gray-800 focus:outline-none" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-bold text-gray-500 uppercase">1 Person (₹)</label>
                        <input type="number" min="0" value={pricingForm.doubleBed?.onePerson ?? ""} onChange={e => setPricingForm((p: any) => ({...p, doubleBed: {...p.doubleBed, onePerson: e.target.value}}))} className="w-full p-2 border rounded mt-1 font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-gray-500 uppercase">2 Persons (₹)</label>
                        <input type="number" min="0" value={pricingForm.doubleBed?.twoPerson ?? ""} onChange={e => setPricingForm((p: any) => ({...p, doubleBed: {...p.doubleBed, twoPerson: e.target.value}}))} className="w-full p-2 border rounded mt-1 font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-gray-500 uppercase">3 Persons (₹)</label>
                        <input type="number" min="0" value={pricingForm.doubleBed?.threePerson ?? ""} onChange={e => setPricingForm((p: any) => ({...p, doubleBed: {...p.doubleBed, threePerson: e.target.value}}))} className="w-full p-2 border rounded mt-1 font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-gray-500 uppercase">4 Persons (₹)</label>
                        <input type="number" min="0" value={pricingForm.doubleBed?.fourPerson ?? ""} onChange={e => setPricingForm((p: any) => ({...p, doubleBed: {...p.doubleBed, fourPerson: e.target.value}}))} className="w-full p-2 border rounded mt-1 font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                      </div>
                    </div>
                    <div className="bg-orange-50 p-3 rounded-lg border border-orange-100">
                      <label className="text-xs font-bold text-orange-800 uppercase">Extra Child Rate (₹)</label>
                      <input type="number" min="0" value={pricingForm.doubleBed?.extraChild ?? ""} onChange={e => setPricingForm((p: any) => ({...p, doubleBed: {...p.doubleBed, extraChild: e.target.value}}))} className="w-full p-2 border border-orange-200 rounded mt-1 font-bold text-gray-800 focus:outline-none" />
                    </div>
                  </div>
                ...
                <div className="mt-8 border-t border-gray-200 pt-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="font-serif text-xl font-bold text-brand-text">Meal Plan Add-ons</h4>
                  </div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">Pricing per guest, per night (₹)</p>
                  
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                      <label className="text-xs font-bold text-gray-700 block mb-2">EP (Room Only)</label>
                      <input type="number" min="0" value={pricingForm.mealPlans?.ep ?? ""} onChange={e => setPricingForm((p: any) => ({...p, mealPlans: {...p.mealPlans, ep: e.target.value}}))} className="w-full p-2 border border-gray-300 rounded font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                      <label className="text-xs font-bold text-gray-700 block mb-2">CP (Breakfast)</label>
                      <input type="number" min="0" value={pricingForm.mealPlans?.cp ?? ""} onChange={e => setPricingForm((p: any) => ({...p, mealPlans: {...p.mealPlans, cp: e.target.value}}))} className="w-full p-2 border border-gray-300 rounded font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                      <label className="text-xs font-bold text-gray-700 block mb-2">MAP (2 Meals)</label>
                      <input type="number" min="0" value={pricingForm.mealPlans?.map ?? ""} onChange={e => setPricingForm((p: any) => ({...p, mealPlans: {...p.mealPlans, map: e.target.value}}))} className="w-full p-2 border border-gray-300 rounded font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                      <label className="text-xs font-bold text-gray-700 block mb-2">AP (3 Meals)</label>
                      <input type="number" min="0" value={pricingForm.mealPlans?.ap ?? ""} onChange={e => setPricingForm((p: any) => ({...p, mealPlans: {...p.mealPlans, ap: e.target.value}}))} className="w-full p-2 border border-gray-300 rounded font-bold text-gray-800 focus:outline-none focus:border-brand-primary" />
                    </div>
                  </div>
                </div>
